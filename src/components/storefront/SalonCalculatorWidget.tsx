...
interface SalonCalculatorProps {
  defaultChairPriceNpr?: number;
  equipmentType?: string;
}

export const SalonCalculatorWidget: React.FC<SalonCalculatorProps> = ({
  defaultChairPriceNpr = 35000,
  equipmentType = "Eternity Salon & Spa Equipment",
}) => {
  const [chairsCount, setChairsCount] = useState<number>(3);
  const [dailyClients, setDailyClients] = useState<number>(6);
  const [avgPrice, setAvgPrice] = useState<number>(800);
  ...
  // Total Fit-Out Investment Estimate = chairs * defaultChairPriceNpr
  const totalInvestmentNpr = chairsCount * defaultChairPriceNpr;

  // Payback Days = totalInvestment / dailyRevenue
  const paybackDays = Math.max(1, Math.ceil(totalInvestmentNpr / Math.max(1, dailyRevenueNpr)));
  const paybackMonths = (paybackDays / 30).toFixed(1);

  return (
    <div className="rounded-3xl bg-inverse-surface text-inverse-on-surface p-6 sm:p-10 border border-neutral-800 shadow-elevated space-y-8 my-10">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-neutral-800 pb-6">
        <div>
          ...
          <p className="text-xs text-neutral-400 font-light mt-1">
            Estimate your monthly revenue & payback timeline when equipping your parlour with {equipmentType}.
          </p>
        </div>
        <div className="px-3.5 py-1.5 rounded-full bg-gold/15 text-gold border border-gold/40 text-xs font-bold flex items-center">
          <Sparkles className="w-3.5 h-3.5 mr-1" /> Live ROI Predictor
        </div>
      </div>
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-xs font-semibold">
              <span className="text-neutral-400">Number of Luxury Equipment Units:</span>
              <span className="text-gold font-bold font-mono text-sm">{chairsCount} Units</span>
            </div>
            <input
              type="range"
              min={1}
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-xs font-semibold">
              <span className="text-neutral-400">Daily Clients / Spa Services per Unit:</span>
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-xs font-semibold">
              <span className="text-neutral-400">Average Charge per Service (NPR):</span>
        ...
        <div className="lg:col-span-5 p-6 rounded-2xl bg-neutral-900 border border-neutral-800 space-y-4">
          ...
          <div className="p-4 rounded-xl bg-gold/10 border border-gold/40 space-y-1">
            ...
            <p className="text-[11px] text-neutral-300">
              Your investment pays for itself completely in ~{paybackMonths} months based on daily client throughput.
            </p>
          </div>

          <div className="text-xs text-neutral-400 space-y-2 pt-1">
            <div className="flex items-center text-white font-semibold">
              <CheckCircle2 className="w-4 h-4 text-gold mr-1.5" /> 15% Wholesale Tier Discount Available
            </div>
            <a
